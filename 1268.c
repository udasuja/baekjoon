...
int main(void)
{
	...
	scanf_s("%d", &n);
	student = (STU*)malloc(sizeof(int) * n);//student는 STU자료형에 대한 구조체 변수이기에 STU*를 써야한다.
...
void student_count(STU* student,int n)//1번 학생부터 1학년부터 5학년중 같은 반인 애가 있는지 검사하고 그다음 2번 이런식으로 올라간다.
{
	...
	while (index<n)
	{
		...
		{
			for (k = 0; k < n; k++)//1번부터 n번까지 index번과 검사하여 서로 같으면 index count에 1추가한다.
			{
				if(k!=index)//k==index일때는 무조건 count가 1추가되므로
